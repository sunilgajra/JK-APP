      ...
      <form id="po-form" class="item mt-12">
        <div class="grid gap-15">
          <div class="grid grid-3 gap-10">
            ...
            <div>
              <label class="form-label">Link to Deals (Hold Ctrl/Cmd for multiple)</label>
              <select name="deal_id" id="po-deal-select" multiple style="height:80px">
...
export function bindPOUI() {
  ...
  document.getElementById("po-deal-select")?.addEventListener("change", (e) => {
    ...
    if (opts.length > 0) {
      ...
      opts.forEach((id, idx) => {
        ...
        if (deal) {
          if (idx === 0) firstDeal = deal;
          totalQty += Number(deal.quantity || 0);
        }
      });

      if (firstDeal) {
        let approxQty = totalQty;
        if (totalQty >= 100) approxQty = Math.round(totalQty / 100) * 100;
        else if (totalQty >= 10) approxQty = Math.round(totalQty / 10) * 10;
        else approxQty = Math.round(totalQty);
        
        let packing = firstDeal.loaded_on || firstDeal.package_details || "";

        form.elements["supplier_id"].value = firstDeal.supplier_id || "";
        form.elements["product_name"].value = opts.length > 1 ? "MULTIPLE PRODUCTS" : (firstDeal.product_name || "");
        form.elements["hsn_code"].value = firstDeal.hsn_code || "";
        if (form.elements["packing"]) form.elements["packing"].value = packing;
        form.elements["quantity"].value = "Approx " + approxQty + " " + (firstDeal.unit || "MT");
        form.elements["price"].value = (firstDeal.document_currency || "USD") + " " + (firstDeal.purchase_rate || firstDeal.rate || 0);
        form.elements["incoterm"].value = (firstDeal.terms_delivery || "") + " " + (firstDeal.discharge_port || "");
        updatePONumber();
      }
    }
  });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const selectedDeals = fd.getAll("deal_id");
    if (selectedDeals.length > 0) {
      data.deal_id = selectedDeals[0]; 
      data.linked_deals = selectedDeals.join(","); // Save directly to the new text column
    } else {
      data.deal_id = null;
      data.linked_deals = null;
    }
